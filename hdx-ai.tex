...
\begin{frame}{The OT Security Tetrad: Adding Safety}
    ...
    \begin{tikzpicture}[scale=0.5]
        ...
        % With minimum size=2.2cm and scale=0.5, effective radius ~0.55cm
        % Need separation > 2*radius = 1.1cm scaled, so use 3.5 units at scale 0.5

        % Diamond arrangement with proper spacing
        % Top - Confidentiality
        \node[circle, fill=hdxpurple, text=white, minimum size=2.2cm, font=\small\bfseries] (conf) at (0,3.5) {Confidentiality};

        % Left - Integrity
        \node[circle, fill=hdxpurple, text=white, minimum size=2.2cm, font=\small\bfseries] (int) at (-3.5,0) {Integrity};

        % Right - Availability
        \node[circle, fill=hdxpurple, text=white, minimum size=2.2cm, font=\small\bfseries] (avail) at (3.5,0) {Availability};

        % Bottom - Safety (highlighted)
        \node[circle, fill=hdxgreen, text=white, minimum size=2.2cm, font=\small\bfseries] (safe) at (0,-3.5) {Safety};
