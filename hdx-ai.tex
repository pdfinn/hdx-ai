...
\begin{frame}{The CIA Triad: Foundation of Information Security}
    ...
    \begin{tikzpicture}
        ...
        % All circles same size: 1.8cm diameter
        % Triangle layout with equal spacing

        % Top circle - Confidentiality (at apex of triangle)
        \node[circle, fill=hdxpurple, text=white, minimum size=1.8cm, font=\scriptsize\bfseries] (conf) at (0,1.6) {Confidentiality};

        % Bottom left circle - Integrity
        \node[circle, fill=hdxpurple, text=white, minimum size=1.8cm, font=\scriptsize\bfseries] (int) at (-1.8,-0.8) {Integrity};

        % Bottom right circle - Availability
        \node[circle, fill=hdxpurple, text=white, minimum size=1.8cm, font=\scriptsize\bfseries] (avail) at (1.8,-0.8) {Availability};
...
\begin{frame}{The OT Security Tetrad: Adding Safety}
    ...
    \begin{tikzpicture}
        ...
        % Safety in center (larger, emphasized as most important)
        \node[circle, fill=hdxgreen, text=white, minimum size=2.2cm, font=\small\bfseries] (safe) at (0,0) {Safety};

        % CIA elements orbit around Safety (smaller circles)
        % Top - Confidentiality
        \node[circle, fill=hdxpurple, text=white, minimum size=1.6cm, font=\scriptsize\bfseries] (conf) at (0,2.2) {Confidentiality};

        % Bottom left - Integrity
        \node[circle, fill=hdxpurple, text=white, minimum size=1.6cm, font=\scriptsize\bfseries] (int) at (-1.9,-1.1) {Integrity};

        % Bottom right - Availability
        \node[circle, fill=hdxpurple, text=white, minimum size=1.6cm, font=\scriptsize\bfseries] (avail) at (1.9,-1.1) {Availability};

        % Connecting lines from CIA to Safety (Safety is central)
        \draw[thick, hdxgreen] (conf) -- (safe);
        \draw[thick, hdxgreen] (int) -- (safe);
        \draw[thick, hdxgreen] (avail) -- (safe);

        % CIA interconnections (lighter)
        \draw[thick, hdxpurple!60] (conf) -- (int);
        \draw[thick, hdxpurple!60] (int) -- (avail);
        \draw[thick, hdxpurple!60] (avail) -- (conf);
    \end{tikzpicture}

    \vspace{2mm}

    {\small In Operational Technology (OT) and AI systems, \textbf{Safety} becomes central:\\preventing harm to people, property, and the environment.}
\end{frame}
