...
\begin{frame}{The CIA Triad: Foundation of Information Security}
    ...
    \begin{tikzpicture}
        ...
        % Circle radius = 1.1cm, so diameter = 2.2cm
        % For non-overlapping: center distance > 2*radius = 2.2cm
        % Using 2.4cm separation for slight gap

        % Top circle - Confidentiality (at apex of triangle)
        \node[circle, fill=hdxpurple, text=white, minimum size=2.2cm, font=\small\bfseries] (conf) at (0,1.4) {Confidentiality};

        % Bottom left circle - Integrity
        \node[circle, fill=hdxpurple, text=white, minimum size=2.2cm, font=\small\bfseries] (int) at (-2.1,-1.1) {Integrity};

        % Bottom right circle - Availability
        \node[circle, fill=hdxpurple, text=white, minimum size=2.2cm, font=\small\bfseries] (avail) at (2.1,-1.1) {Availability};
...
\begin{frame}{The OT Security Tetrad: Adding Safety}
    ...
    \begin{tikzpicture}
        ...
        % Circle diameter = 2cm, radius = 1cm
        % For non-overlapping diamond: need center distance > 2cm
        % Using 2.3cm between adjacent circles

        % Top - Confidentiality
        \node[circle, fill=hdxpurple, text=white, minimum size=2cm, font=\small\bfseries] (conf) at (0,2.3) {Confidentiality};

        % Left - Integrity
        \node[circle, fill=hdxpurple, text=white, minimum size=2cm, font=\small\bfseries] (int) at (-2.3,0) {Integrity};

        % Right - Availability
        \node[circle, fill=hdxpurple, text=white, minimum size=2cm, font=\small\bfseries] (avail) at (2.3,0) {Availability};

        % Bottom - Safety (highlighted)
        \node[circle, fill=hdxgreen, text=white, minimum size=2cm, font=\small\bfseries] (safe) at (0,-2.3) {Safety};
