...
\begin{frame}{Your Instructor}
    \begin{columns}[c]
        ...
        \begin{column}{0.58\textwidth}
            ...
            \includegraphics[height=0.6cm]{assets/logos/National_University_of_Singapore-Logo.png}\hspace{2mm}%
            \includegraphics[height=0.6cm]{assets/logos/KCL Logo.png}

            \vspace{2mm}

            \textbf{Co-chair}, Emerging Technology \& Digital Leadership\\
            Golden Gate University\\[1mm]
            \includegraphics[height=0.7cm]{assets/logos/GGU-Logo.png}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Case Study: AI-Controlled Chemical Synthesis}
    \textbf{Coscientist (Carnegie Mellon, 2023)}

    \vspace{2mm}

    Researchers built an autonomous AI agent that controls real laboratory equipment:

    \vspace{2mm}

    \begin{itemize}
        \item GPT-4 powered agent with access to internet search, documentation, code execution
        \item Connected to physical lab automation (liquid handlers, synthesis equipment)
        \item Given plain-language prompts, it designs, plans, and \textit{executes} experiments
    \end{itemize}

    \vspace{2mm}

    \textbf{The problem}: When prompted, Coscientist could produce:
    \begin{itemize}
        \item Controlled drugs
        \item Chemical weapons precursors
    \end{itemize}

    \vspace{2mm}

    \begin{alertblock}{This is Real, Not Hypothetical}
        An agentic AI connected to lab equipment can autonomously synthesize dangerous substances. The design-make-test cycle is no longer theoretical.
    \end{alertblock}

    \vspace{1mm}

    {\tiny Source: Boiko et al., ``Autonomous chemical research with large language models,'' \textit{Nature} 624, 570--578 (2023)}
\end{frame}
...
\begin{frame}{Defence in Depth: Guardrails in Practice}
    ...
    \begin{tikzpicture}[scale=0.65, every node/.style={font=\footnotesize}]
        ...
        \node[font=\tiny\itshape, text=hdxblue!90, below=2pt of llm] {RLHF + System Prompt};
...
\begin{frame}{Why Multiple Layers Matter}
    ...
    \begin{tabular}{p{3cm}p{4.5cm}p{4.5cm}}
        ...
        LLM guardrails & Contextual threats & Jailbreaks (as you saw!) \\
    ...
    \begin{alertblock}{The CTF Lesson}
        You bypassed LLM guardrails in minutes. That's why we need layers \textit{before} and \textit{after} the model too.
    \end{alertblock}
\end{frame}
